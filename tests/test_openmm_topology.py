...
def test_func_subset_polymer():
    ...
    topo_sub, pos_sub = t.subset(topology, positions)
    ...
    topo_sub, pos_sub = t.subset(topology, positions, delete=atoms[n_atoms:])
    ...
    topo_sub, pos_sub = t.subset(topology, positions, keep=np.arange(n_atoms), 
                                 types="atom")
    assert topo_sub.getNumAtoms() == n_atoms
    assert topo_sub.getNumBonds() == n_bonds
    assert topo_sub.getNumResidues() == n_atoms
    ...
    topo_sub, pos_sub = t.subset(topology, positions, keep=np.arange(n_bonds), 
                                 types=n_bonds * ["bond"])
    assert topo_sub.getNumAtoms() == n_atoms
    assert topo_sub.getNumBonds() == n_bonds
    assert topo_sub.getNumResidues() == n_atoms
    ...
    topo_sub, pos_sub = t.subset(topology, positions, keep=np.arange(n_chains), 
                                 types="chain")
    assert topo_sub.getNumAtoms() == n_atoms
    assert topo_sub.getNumBonds() == n_bonds
    assert topo_sub.getNumResidues() == n_atoms
